...
const UserProfile = () => {
    ...
    React.useEffect(() => { axios.get(`api/user/getNewOrderInfo?email=${localStorage.getItem("email")}`).then(({ data }) => setNewOrder(data)) }, [canReloadOrder])
                    ...
                    <div>
                        {
                            newOrder?.status == "Ожидает оплаты"
                                ...
                                <div><button type="submit" className={style.submit} onClick={() => {
                                    axios.get(`/api/user/getStatusInProcess?email=${localStorage.getItem("email")}`)
                                    setCanReloadOrder(prevState => !prevState)
                                }}><span>Оплатить</span></button>
                                    <button type="submit" className={style.submit_cancel} onClick={() => {
                                        axios.get(`/api/user/getStatusDelete?email=${localStorage.getItem("email")}`)
                                        setCanReloadOrder(prevState => !prevState)
                                    }
